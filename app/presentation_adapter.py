...
def dashboard_summary(db: Session = Depends(get_db)):
    try: products_total = db.query(models.Product).count()
    except Exception: products_total = 0
    try: vendors_total = db.query(models.Vendor).count()
    except Exception: vendors_total = 0
...
def bills_recent(db: Session = Depends(get_db)):
    try:
        Bill = models.Bill
        date_col = getattr(Bill, "bill_date")
        total_col = getattr(Bill, "total")
        items_count = dict(
            db.query(models.BillLine.bill_id, func.count(models.BillLine.id)).group_by(models.BillLine.bill_id).all()
        )
        q = db.query(Bill).order_by(date_col.desc()).limit(10).all()
        out: List[BillRow] = []
        for b in q:
            vendor_name = b.vendor.name if b.vendor else (b.party_name or "—")
            dt_str = b.bill_date.isoformat()
            total_val = float(total_col.__get__(b, Bill) or 0)
            status = b.status or ("Processed" if total_val > 0 else "Pending")
            out.append(BillRow(
                bill_no=b.bill_no or str(b.id),
                vendor=vendor_name,
                date=dt_str,
                items=int(items_count.get(b.id, 0)),
                total=total_val,
                status=status
            ))
        if out:
...
def list_products(q: Optional[str] = None, category: Optional[str] = None, db: Session = Depends(get_db)):
    try:
        ...
        out: List[ProductOut] = []
        for p in items:
            stock_val = getattr(p, "stock_qty", 0)
            price_val = getattr(p, "price", 0.0)
